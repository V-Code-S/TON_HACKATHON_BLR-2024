...
cp -f /var/www/MeTon/conf/nginx-site.conf /etc/nginx/sites-enabled/meton.conf
cp -f /var/www/MeTon/conf/nginx.conf /etc/nginx/nginx.conf
...
cd /var/www/MeTon/engine
composer install
cd -

# Additional folders
mkdir --parents --mode=0777 /tmp/meton-cache/
mkdir --parents --mode=0777 /data/

cd /var/www/MeTon

if [ -f "/var/www/MeTon/engine/settings.php" ]; then
  echo "Provisioning MeTon…"
  ...
  php ./bin/cli.php install \
    ...
    --username=meton \
    --password=password \
    --email=meton@dev.metons.io
else
  echo "Installing MeTon…"

  php ./bin/regenerateDevKeys.php;

  php ./bin/cli.php install \
    --graceful-storage-provision \
    --domain=dev.metons.io \
    --username=meton \
    --password=password \
    --email=meton@dev.metons.io \
    --private-key=/var/www/MeTon/.dev/meton.pem \
    --public-key=/var/www/MeTon/.dev/meton.pub
fi
