...
switch($argv[1]){
    ...
    case "signups":
        ...
        $db = new MeTon\Core\Data\Call('entities_by_time');
        $limit = 1000;
        $offset = "";
        while(true){
            echo "[$offset]: ";
            $users = MeTon\Core\Entities::get(['type'=>'user', 'limit' => $limit, 'offset'=>$offset]);
            foreach($users as $user){
                $ts = MeTon\Core\Analytics\Timestamps::get(['day', 'month'], $user->time_created);
    ...
    case "retention":
        ...
        $app = MeTon\Core\Analytics\App::_()
          ->setMetric('retention')
          ->increment();
        echo "Done \n";
        break;
