...
try {
    $handler = MeTon\Cli\Factory::build($argv);

    if (!$handler) {
        echo "{$_SCRIPTNAME}: controller `{$argv[0]}` not found" . PHP_EOL;
        exit(1);
    } elseif (!($handler instanceof MeTon\Interfaces\CliControllerInterface)) {
...
} catch (MeTon\Exceptions\CliException $e) {
